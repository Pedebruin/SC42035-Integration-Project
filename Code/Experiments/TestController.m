...
switch TypeController
    ...
    case 'hinf'
        ...
        L = LQRd(sys,[0;0]);                % Calculate L for observer. 
        xhat_k0 = [0,0,0,0,0]';             % Observer initialisation.
        yHinf_k0 = [0;0];                   % Initial output is zero. 
    otherwise
...
switch TypeSignal
    case 'step'
        H1Ref = [ 50]; %reference signal without multiplier.
        H2Ref = [ 40]; %reference signal without multiplier.
        ...
        switch TypeController
            ...
            case 'hinf'
                ...
                if i <= InitialRestTime*60 %for safety of observer, give time to adjust + temperature read.
                    ...
                    % ---- Hinf state space: ----
                    xHinf_k1 = K_Hinf.A*xHinf_k0 + K_Hinf.B*(r - yhat_k0); % Simulate the dynamic controller, can also use yhat_k0
                    yHinf_k0 = K_Hinf.C*xHinf_k0 + K_Hinf.D*(r - yhat_k0); 
